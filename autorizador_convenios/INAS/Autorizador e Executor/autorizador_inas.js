...
const listaCodigos=entrada.split([s,;n]+).map(c=c.trim()).filter(Boolean);
...
['mousedown','mouseup','click'].forEach(evt=seta.dispatchEvent(new MouseEvent(evt,{bubblestrue,cancelabletrue})));
...
['input','change','blur'].forEach(evt=campoQuantidade.dispatchEvent(new Event(evt,{bubblestrue})));
return true;}
...
const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;
setter.call(campo,'');
campo.dispatchEvent(new Event('input',{bubblestrue}));
setter.call(campo,codigo);
campo.dispatchEvent(new InputEvent('input',{bubblestrue,datacodigo,inputType'insertText'}));
